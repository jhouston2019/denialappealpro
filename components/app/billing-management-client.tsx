  ...
  return (
    <div style={styles.container}>
        ...
        {subscription ? (
          <div>
            <div style={styles.planInfo}>
              <div style={styles.planName}>{formatPlanName(subscription.plan)}</div>
              <div style={styles.planPrice}>${planDetails?.price}/month</div>
            </div>
            <div style={styles.planDetails}>
              <div style={styles.detailRow}>
                <span>Status:</span>
                <span style={statusBadgeStyle(subscription.status)}>{subscription.status}</span>
              </div>
              <div style={styles.detailRow}>
            ...
            <button type="button" onClick={() => void handleManageBilling()} style={styles.primaryButton}>
              Manage Subscription
            </button>
            <p style={styles.helperText}>Upgrade, downgrade, update payment method, or cancel</p>
          </div>
        ) : (
          <div>
            <p style={styles.noSubscription}>You don&apos;t have an active subscription</p>
            <button type="button" onClick={() => router.push("/pricing")} style={styles.primaryButton}>
              View Plans
            </button>
          </div>
        )}
      ...
      {usageStats && (
        <div style={styles.card}>
          ...
          <div style={styles.usageBar}>
            <div style={usageBarFill(usagePercentage)} />
          </div>
          ...
          {overageCount > 0 && (
            <div style={styles.overageBox}>
              ...
              <div style={styles.overageCost}>Additional charge: ${(overageCount * 0.5).toFixed(2)}</div>
      ...
      <div style={styles.actions}>
        <button type="button" onClick={() => router.push("/start")} style={styles.secondaryButton}>
          Process New Denial
        </button>
        <button type="button" onClick={() => router.push("/pricing")} style={styles.secondaryButton}>
          View All Plans
        </button>
      </div>
    </div>
...
function usageBarFill(percentage: number): React.CSSProperties {
  return {
    height: "100%",
...
const styles: Record<string, React.CSSProperties> = {
  ...
  title: { fontSize: "36px", marginBottom: "10px", color: TEXT_ON_SLATE },
  subtitle: { fontSize: "18px", color: TEXT_MUTED_ON_SLATE },
  card: {
    background: "white",
    borderRadius: "12px",
    padding: "30px",
  ...
  cardTitle: { fontSize: "24px", marginBottom: "20px", color: "#2c3e50" },
  planInfo: {
    display: "flex",
  ...
  planName: { fontSize: "28px", fontWeight: "bold" },
  planPrice: { fontSize: "32px", fontWeight: "bold" },
  planDetails: { marginBottom: "20px" },
  detailRow: {
    display: "flex",
    justifyContent: "space-between",
    padding: "12px 0",
  ...
  helperText: { textAlign: "center", fontSize: "14px", color: TEXT_MUTED_ON_SLATE, marginTop: "10px" },
  noSubscription: { textAlign: "center", fontSize: "16px", color: TEXT_MUTED_ON_SLATE, marginBottom: "20px" },
  usageBar: {
    width: "100%",
    height: "12px",
    background: "#e5e7eb",
  ...
  usageStats: { display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "20px", marginBottom: "20px" },
  usageStat: { textAlign: "center" },
  usageNumber: { fontSize: "32px", fontWeight: "bold", color: "#2c3e50" },
  usageLabel: { fontSize: "14px", color: "#475569", marginTop: "5px" },
  overageBox: {
    background: "#fef3c7",
  ...
  overageTitle: { fontSize: "18px", fontWeight: "bold", color: "#92400e", marginBottom: "10px" },
  overageText: { fontSize: "16px", color: "#92400e", marginBottom: "10px" },
  overageCost: { fontSize: "24px", fontWeight: "bold", color: "#92400e", marginBottom: "10px" },
  overageNote: { fontSize: "14px", color: "#92400e", marginTop: "10px" },
  warningBox: {
    background: "#fef3c7",
  ...
  statNumber: { fontSize: "48px", fontWeight: "bold", color: "#667eea" },
  statLabel: { fontSize: "16px", color: "#475569", marginTop: "10px" },
  actions: { display: "flex", justifyContent: "center", gap: "10px" },
  loading: { textAlign: "center", fontSize: "18px", color: TEXT_MUTED_ON_SLATE, padding: "40px" },
  error: { textAlign: "center", fontSize: "18px", color: "#fb923c", padding: "40px", marginBottom: "20px" },
  button: {
    display: "block",
    margin: "0 auto",
    padding: "12px 24px",
